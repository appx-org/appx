...
The server detects message type: valid JSON with `"type":"resize"` is a control message; everything else is raw terminal bytes. This keeps the fast path (I/O) as zero-copy byte forwarding with no framing overhead.
